...
(defrules rules
  ...
  ; Same as for inheritance again
  #R[(M ==> P) (S <=> M) |- (S ==> P) :pre (#(not= S P)) :post (:t/analogy :allow-backward)]
  #R[(M =/> P) (S </> M) |- (S =/> P) :pre (#(not= S P)) :post (:t/analogy :allow-backward)]
  #R[(M =/> P) (S <|> M) |- (S =/> P) :pre (#(not= S P)) :post (:t/analogy :allow-backward)]
  #R[(M =|> P) (S <|> M) |- (S =|> P) :pre (#(not= S P)) :post (:t/analogy :allow-backward)]
  #R[(M =\> P) (M </> S) |- (S =\> P) :pre (#(not= S P)) :post (:t/analogy :allow-backward)]
  #R[(M =\> P) (S <|> M) |- (S =\> P) :pre (#(not= S P)) :post (:t/analogy :allow-backward)]

  #R[(P ==> M) (S <=> M) |- (P ==> S) :pre (#(not= S P)) :post (:t/analogy :allow-backward)]
  #R[(P =/> M) (S <|> M) |- (P =/> S) :pre (#(not= S P)) :post (:t/analogy :allow-backward)]
  #R[(P =|> M) (S <|> M) |- (P =|> S) :pre (#(not= S P)) :post (:t/analogy :allow-backward)]
  #R[(P =\> M) (S </> M) |- (P =\> S) :pre (#(not= S P)) :post (:t/analogy :allow-backward)]
  #R[(P =\> M) (S <|> M) |- (P =\> S) :pre (#(not= S P)) :post (:t/analogy :allow-backward)]

  #R[(M <=> P) (S <=> M) |- (S <=> P) :pre (#(not= S P)) :post (:t/resemblance :order-for-all-same :allow-backward)]
  #R[(M </> P) (S <|> M) |- (S </> P) :pre (#(not= S P)) :post (:t/resemblance :allow-backward)]
  #R[(M <|> P) (S </> M) |- (S </> P) :pre (#(not= S P)) :post (:t/resemblance :allow-backward)]
  ...
  #R[(P =|> M) (S =|> M) |- (((P || S) =|> M) :post (:t/intersection)
                              ((P &| S) =|> M) :post (:t/union))
     :pre (#(not= S P))]
  #R[(P =/> M) (S =/> M) |- (((P || S) =/> M) :pre (:t/intersection)
                              ((P &| S) =/> M) :post (:t/union))
     (#(not= S P)) :post]
  #R[(P =\> M) (S =\> M) |- (((P || S) =\> M) :post (:t/intersection)
                                ((P &| S) =\> M) :post (:t/union))
  ...
  #R[(&/ :list/A) A_1 |- A_1 :post (:t/structural-deduction :d/structural-strong)]
  #R[(&| :list/A) A_1 |- A_1 :post (:t/structural-deduction :d/structural-strong)]
  #R[(&/B :list/A) B |- (&/ :list/A) :pre [(task "!")] :post (:t/deduction :d/strong :seq-interval-from-premises)]
  ...
  #R[(S --> M) (P --> M) |- (((P --> $X) =|> (S --> $X)) :post (:t/abduction :linkage-temporal)
            ...
            :pre (#(not= S P) (concurrent Task Belief))]
  ...
  #R[(M --> S) (M --> P) |- ((($X --> S) =|> ($X --> P)) :post (:t/induction :linkage-temporal)
     ...
     :pre (#(not= S P) (concurrent (M --> P) (M --> S)))]
